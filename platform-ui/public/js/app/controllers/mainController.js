...
app.config(function($stateProvider) {
  $stateProvider
    .state('learningMap', {
      ...
      url: "/langSearch/:id/:name",
      views: {
        "contentSection": {
          templateUrl: contextPath + "/templates/player/languageSearch.html",
          controller: 'LangSearchCtrl'
        },
      },
      onEnter: function() {
        $(".RightSideBar").addClass('Effectsidebar').css('display', 'none');
...
app.controller('PlayerController', ['$scope', '$timeout', '$rootScope', '$stateParams', '$state', 'PlayerService', '$location', '$anchorScroll', '$sce', function($scope, $timeout, $rootScope, $stateParams, $state, service, $location, $anchorScroll, $sce) {
  ...
  $scope.selectLanguage = function(languageId, languageName) {
    $state.go('langSearch', {
      id: languageId,
      name: languageName
    });
  ...
  $scope.languageSearch = function() {
    console.log("Search Function Executed");
      ...
      var index = 1;
      $scope.commitMessage = "Following is the Search Criteria:\n";

      for (k in $scope.selectedConcept.metadata) {
        var prop = _.where($scope.selectedTaxonomy.properties, {
          'propertyName': k
        })[0];
        ...
        if (_.isArray(newValue)) {
          ...
          if (_.difference(oldValue, newValue).length > 0 || _.difference(newValue, oldValue).length > 0) {
            $scope.commitMessage += index++ + '. "' + prop.title + '" value is "' + newValue + '"\n';
      ...
      console.log("Search Request Object - ", $scope.conceptToBeUpdated);
  ...
  $scope.resetRelations = function() {
    ...
    _.each($scope.selectedTaxonomy.definitions.inRelations, function(defRel) {
      var inRelObjects = _.map(_.where($scope.selectedConcept.inRelations, {
        "relationType": defRel.relationName
      }), function(inRel) {
        return {
          id: inRel.startNodeId,
          name: inRel.startNodeName,
          direction: "in",
          relationType: inRel.relationType,
          objectType: inRel.startNodeObjectType
        };
      });
    ...
    _.each($scope.selectedTaxonomy.definitions.outRelations, function(defRel) {
      var outRelObjects = _.map(_.where($scope.selectedConcept.outRelations, {
        "relationType": defRel.relationName
      }), function(outRel) {
        return {
          id: outRel.endNodeId,
          name: outRel.endNodeName,
          direction: "out",
          relationType: outRel.relationType,
          objectType: outRel.endNodeObjectType
        };
      });
      relations[defRel.title] = _.reject(outRelObjects, function(r) {
        return r.objectType == "Media";
      });
    });
...
app.controller('LearningMapController', ['$scope', '$timeout', '$rootScope', '$stateParams', '$state', 'PlayerService', function($scope, $timeout, $rootScope, $stateParams, $state, service) {
  ...
  $scope.getTaxonomyDefinitions = function(taxonomyId) {
    service.getTaxonomyDefinitions(taxonomyId).then(function(taxonomyDefs) {
      ...
      _.each(categories, function(category) {
        definitions[category] = _.where(taxonomyDefs.properties, {
          'category': category
        });
      });
  ...
  $rootScope.isNameExist = function(conceptName, oldConceptName) {
    ...
    if (oldConceptName) allConcepts = _.without(allConcepts, _.findWhere(allConcepts, {
      name: oldConceptName
    }));
    var concept = _.findWhere(allConcepts, {
      name: conceptName
    });
  ...
  $rootScope.isCodeExist = function(code, oldCode) {
    ...
    var allConcepts = $scope.allConcepts;
    if (oldCode) allConcepts = _.without(allConcepts, _.findWhere(allConcepts, {
      code: oldCode
    }));
    var concept = _.findWhere(allConcepts, {
      code: code
  ...
  $scope.setTaxonomyGroups = function(graph, allNodes) {
    ...
    if (graph.children && graph.children.length > 0) {
      _.each(graph.children, function(node) {
        ...
        if (node.level == 2) {
          $scope.subConcepts.push({
            name: node.name,
            id: node.conceptId
          });
        }
        if (node.level == 3) {
          $scope.microConcepts.push({
            name: node.name,
            id: node.conceptId
          });
        } else {
...
app.controller('GameController', ['$scope', '$timeout', '$rootScope', '$stateParams', '$state', 'PlayerService', function($scope, $timeout, $rootScope, $stateParams, $state, service) {
  ...
  $scope.viewGameList = function() {
    $state.go('gameList', {
      id: $scope.$parent.selectedTaxonomyId
    });
  }
...
app.controller('ContentController', ['$scope', '$timeout', '$rootScope', '$stateParams', '$state', 'PlayerService', function($scope, $timeout, $rootScope, $stateParams, $state, service) {
  ...
  $scope.setContentResponse = function(data) {
    console.log(data);
    $scope.$parent.selectedConcept = data;
    $scope.$parent.selectedConcept.newMetadata = [];
    $scope.$parent.selectedConcept.relatedConceptsLimit = service.rhsSectionObjectsLimit;
...
app.controller('LangSearchCtrl', ['$scope', '$timeout', '$rootScope', '$stateParams', '$state', 'PlayerService', function($scope, $timeout, $rootScope, $stateParams, $state, service) {
  $scope.$parent.validationMessages = [];
  $scope.$parent.selectedLanguageId = $stateParams.id;
  $scope.$parent.selectedLanguageName = $stateParams.name;

  $scope.$parent.isSearchOperation = true;

  $scope.$on("$destroy", function handler() {
    $scope.$parent.isSearchOperation = false;
  });
  ...
  // $scope.$parent.selectedTaxonomy = $scope.$parent.selectedLanguageId;


  $scope.getLangSearchDefinition = function(languageId) {
    service.getLangSearchDefinition(languageId).then(function(langSearchDef) {
      var categories = _.uniq(_.pluck(langSearchDef.properties, 'category'));
      var definitions = {

      }
      _.each(categories, function(category) {
        definitions[category] = _.where(langSearchDef.properties, {
          'category': category
        });
      });
      $scope.$parent.selectedTaxonomy.properties = langSearchDef.properties;
      $scope.$parent.selectedTaxonomy.definitions = definitions;debugger;
      $scope.setLangSearchObj($scope.langSearchReqObj);
    }).catch(function(err) {
      console.log('Error fetching content definitions - ', err);
    });
  }

  $scope.langSearchReqObj = {
    metadata: {
      startsWith: "fskjdf",
      endsWith: "",
      lemmaValue: "",
      minOrthographicComplexity: "",
      maxOrthographicComplexity: "",
      orthographicComplexityValue: "",
      minPhonologicComplexity: "",
      maxPhonologicComplexity: "",
      phonologicComplexityValue: "",
      minSyllableCount: "",
      maxSyllableCount: "",
      SyllableCountValue: "",
      sourceType: "",
  ...
  $scope.setLangSearchObj = function(data) {
    $scope.$parent.selectedConcept = data;
    $scope.$parent.selectedConcept.newMetadata = [];
    $scope.$parent.unmodifiedConcept = angular.copy($scope.$parent.selectedConcept);
  }
  ...
  $scope.getLangSearchDefinition($scope.$parent.selectedLanguageId);
