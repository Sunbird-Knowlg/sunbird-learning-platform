...
Usage:
node AssetImporter.js --help

-d  dry run
-u  user id
-a  owner (asset attributions - commonly applied to all assets being loaded)
-e  environment (prod, qa, dev, sandbox)
-f  output csv
-r  root directory to scan for assets

Successful record identifiers are written to itemImport/assets-success.json file
Errors are written to itemImport/assets-output.json file

Example:
node AssetImporter.js -e prod -u 128 -f itemImport/assets.csv -r ./assets
...
var options = cli.parse({
    dryrun:   ['d', 'Dry run (scan the assets and print to console).'],
    user:     ['u', 'Your user id (will show in My Assets view)', 'string'],
    env:      ['e', 'Environment', 'string', 'prod'],
    attrib:   ['a', 'Asset attributions', 'string', 'None'],
    file:     ['f', 'Output csv file to write', 'file'],
    dir:      ['r', 'Root directory to scan', 'file']
});

if ((!options.file) || (!options.env) || (!options.user) || (!options.dir)) {
    cli.error("Insufficient inputs.");
    cli.fatal("   [AssetImporter --help] for usage help. ");
}

console.log();
console.log('----------------------------------------------------------------');
console.log("             Asset Importer v1.0               ");
console.log('----------------------------------------------------------------');
console.log();
...
var CREATE_ASSET_URL = "/v1/content/${id}";
var UPLOAD_ASSET_URL = "/v1/content/upload/${id}";

var rootDir = options.dir;
var assetsFile = options.file;
