...
public class GetHierarchyOperation extends BaseContentManager {
    ...
    public Response getContentHierarchy(String contentId, String bookMarkId, String mode, List<String> fields)  {
        ...
        if(StringUtils.equalsIgnoreCase("edit", mode)){
            ...
            // Check for Collection mimetypes
            ...
            String hierarchyResponse = getContentBody(node.getIdentifier().concat(".img"));

            long startTime = System.currentTimeMillis();
            if(StringUtils.isNotBlank(hierarchyResponse)){
                try {
                    dataMap.put("children", objectMapper.readValue(hierarchyResponse, List.class));
                } catch (IOException e) {
                        e.printStackTrace();
                }
            }else {
            ...
            TelemetryManager.log("Time to fetchNodes and construct hierarchy: " + (System.currentTimeMillis() -
                    startTime));
            Response response = new Response();
            response.put("content", dataMap);
            response.setParams(getSucessStatus());
