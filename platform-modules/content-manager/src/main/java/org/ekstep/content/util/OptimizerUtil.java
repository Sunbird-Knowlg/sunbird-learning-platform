...
public class OptimizerUtil {
    ...
    public static String[] uploadToAWS(File uploadedFile, String identifier) {
        ...
        try {
            ...
            throw new ServerException(ContentErrorCodes.ERR_CONTENT_UPLOAD_FILE.name(), "Error while uploading the File.", e);
        }
        return urlArray;
    }
    ...
    public static void videoEnrichment(Node node, String tempFolder, File videoFile) throws Exception {
        ...
        try {
            ...
            TelemetryManager.error("videoEnrichment :: Exception while generating thumbnail for content :: " + node.getIdentifier(), e);
        }
        frameGrabber.stop();
        if (null != thumbNail && thumbNail.exists()) {
            ...
            TelemetryManager.info("videoEnrichment :: Thumbnail could not be generated.");
        }
    }
    ...
    private static File fetchThumbNail(String tempFolder, long numberOfFrames, FFmpegFrameGrabber frameGrabber) throws Exception {
        ...
        File thumbnail = null;
        int colorCount = 0;
        int numberOfSampleThumbnails = Platform.config.hasPath("max.sample.thumbnail.image")?
                Platform.config.getInt("max.sample.thumbnail.image"): 5;
        for (int i = 1; i <= numberOfSampleThumbnails; i++) {
            ...
            frameGrabber.setFrameNumber((int) (numberOfFrames / numberOfSampleThumbnails) * i);
            try {
                ...
                TelemetryManager.error("fetchThumbNail :: Exception while generating thumbnail.", e);
