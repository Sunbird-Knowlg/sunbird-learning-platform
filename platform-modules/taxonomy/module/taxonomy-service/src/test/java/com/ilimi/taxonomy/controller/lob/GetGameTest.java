...
public class GetGameTest {
    ...
    public void getConcept() throws Exception {
        ResultActions actions = mockMvc.perform(get("learning-object/G1").param("taxonomyId", "NUMERACY").param("gfields", "name").header("Content-Type", "application/json").header("user-id", "jeetu"));
    ...
    public void emptyTaxonomyId() throws Exception {
        ResultActions actions = mockMvc.perform(get("learning-object/G1").param("taxonomyId", "").param("gfields", "name").header("Content-Type", "application/json").header("user-id", "jeetu"));
        actions.andDo(MockMvcResultHandlers.print());
...
//        Assert.assertEquals("Taxonomy Id is blank", resp.getParams().get("errmsg"));
    ...
    public void withoutTaxonomyId() throws Exception {
        ResultActions actions = mockMvc.perform(get("learning-object/G1").param("gfields", "name").header("Content-Type", "application/json").header("user-id", "jeetu"));
        actions.andDo(MockMvcResultHandlers.print());
        actions.andExpect(status().is(400));   
    ...
    public void gameNotFound() throws Exception {
        ResultActions actions = mockMvc.perform(get("learning-object/G1239").param("gfields", "name").header("Content-Type", "application/json").header("user-id", "jeetu"));
        actions.andDo(MockMvcResultHandlers.print());
