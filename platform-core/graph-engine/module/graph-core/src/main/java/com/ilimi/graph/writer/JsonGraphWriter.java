    ...
    public OutputStream getData() throws Exception {
        ...
        if (nodes != null) {
            for (Node node : nodes) {
                ...
                if (StringUtils.isNotBlank(node.getNodeType()) && node.getNodeType().equals(SystemNodeTypes.DEFINITION_NODE.name())) {
                    ...
                    if(StringUtils.isNotBlank((String)metadata.get(DefinitionNode.INDEXABLE_METADATA_KEY))) {
                        ...
                        try {
                            List<Map<String, Object>> listMap = (List<Map<String, Object>>) mapper.readValue(metaList, List.class);
                            for (Map<String, Object> metaMap : listMap) {
                                indexedMetadata.add((MetadataDefinition) mapper.convertValue(metaMap, MetadataDefinition.class));
                            }
                        } catch (Exception e) {
                        }
                    }
                    ...
                    if(StringUtils.isNotBlank((String)metadata.get(DefinitionNode.NON_INDEXABLE_METADATA_KEY))) {
                        ...
                        try {
                            List<Map<String, Object>> listMap = (List<Map<String, Object>>) mapper.readValue(metaList, List.class);
                            for (Map<String, Object> metaMap : listMap) {
                                nonIndexedMetadata.add((MetadataDefinition) mapper.convertValue(metaMap, MetadataDefinition.class));
                            }
                        } catch (Exception e) {
                        }
                    }
                    ...
                    if(StringUtils.isNotBlank((String)metadata.get(DefinitionNode.IN_RELATIONS_KEY))) {
                        ...
                        try {
                            List<Map<String, Object>> listMap = (List<Map<String, Object>>) mapper.readValue(inRelList, List.class);
                            for (Map<String, Object> metaMap : listMap) {
                                inRelationMetadata.add((RelationDefinition) mapper.convertValue(metaMap, RelationDefinition.class));
                            }
                        } catch (Exception e) {
                        }
                    }
                    ...
                    if(StringUtils.isNotBlank((String)metadata.get(DefinitionNode.OUT_RELATIONS_KEY))) {
                        ...
                        try {
                            List<Map<String, Object>> listMap = (List<Map<String, Object>>) mapper.readValue(outRelList, List.class);
                            for (Map<String, Object> metaMap : listMap) {
                                outRelationMetadata.add((RelationDefinition) mapper.convertValue(metaMap, RelationDefinition.class));
                            }
                        } catch (Exception e) {
                        }
                    }
                    ...
                    definitionNodesList.add(map);
                        ...
                        && (node.getNodeType().equals(SystemNodeTypes.DATA_NODE.name()) || node.getNodeType().equals(
                                SystemNodeTypes.SEQUENCE.name()))) {
