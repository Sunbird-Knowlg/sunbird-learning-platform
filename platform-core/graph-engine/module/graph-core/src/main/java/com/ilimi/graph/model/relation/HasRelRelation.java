...
public class HasRelRelation extends AbstractRelation {
    ...
    public Future<Map<String, List<String>>> validateRelation(Request request) {
        try {
            ...
            Future<String> startNodeMsg = getNodeTypeFuture(startNode, SystemNodeTypes.DEFINITION_NODE.name(), ec);
            futures.add(startNodeMsg);
            Future<String> endNodeMsg = getNodeTypeFuture(endNode, SystemNodeTypes.RELATION_NODE.name(), ec);
