...
public class Neo4JBoltNodeMgrImpl extends BaseDACMgr implements IGraphDACNodeMgr {
	...
	public Response upsertNode(Request request) {
		...
		org.ekstep.graph.dac.model.Node node = (org.ekstep.graph.dac.model.Node) request.get(GraphDACParams.node.name());
	...
	public Response importNodes(Request request) {
		String graphId = (String) request.getContext().get(GraphHeaderParams.graph_id.name());
		List<org.ekstep.graph.dac.model.Node> nodes = (List<org.ekstep.graph.dac.model.Node>) request
