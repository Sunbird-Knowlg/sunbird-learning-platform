...
public class Neo4JEmbeddedNodeOperations {
	...
	public com.ilimi.graph.dac.model.Node upsertNode(String graphId, com.ilimi.graph.dac.model.Node node,
		...
		try (Transaction tx = graphDb.beginTx()) {
			LOGGER.debug("Transaction Started For 'upsertNode' Operation. | [Node ID: '" + node.getIdentifier() + "']");
			String date = DateUtils.formatCurrentDate();
			LOGGER.debug("Date: " + date);
			Node neo4jNode = null;
			try {
				neo4jNode = Neo4jGraphUtil.getNodeByUniqueId(graphDb, node.getIdentifier());
				LOGGER.debug("Fetched Neo4J Node Id: " + neo4jNode.getId());

				validateNodeUpdate(neo4jNode, node);
				LOGGER.debug("Node Update Validated: " + node.getIdentifier());
				...
				LOGGER.debug("Node Doesn't Exist, Creating a New Node. | [Node ID: '" + node.getIdentifier() + "']");
				...
				LOGGER.debug("Setting System Properties For Node. | [Node ID: '" + node.getIdentifier() + "']");
			...
			LOGGER.debug("Setting Node Data. | [Node ID: '" + node.getIdentifier() + "']");
			...
			if (!StringUtils.isBlank(date)) {
				...
				LOGGER.debug("Setting Version Key. | [Node ID: '" + node.getIdentifier() + "']");
				node.getMetadata().put(GraphDACParams.versionKey.name(),
						Long.toString(DateUtils.parse(date).getTime()));
			}
			tx.success();
			LOGGER.debug("Transaction For Operation 'upsertNode' Completed Successfully. | [Node ID: '"
					+ node.getIdentifier() + "']");
			return node;
		}
	}
	...
	public com.ilimi.graph.dac.model.Node addNode(String graphId, com.ilimi.graph.dac.model.Node node,
		...
		GraphDatabaseService graphDb = Neo4jGraphFactory.getGraphDb(graphId, request);
		try (Transaction tx = graphDb.beginTx()) {
			LOGGER.debug("Transaction Started For 'addNode' Operation. | [Node ID: '" + node.getIdentifier() + "']");
			...
			tx.success();
			LOGGER.debug("Transaction For Operation 'addNode' Completed Successfully. | [Node ID: '"
					+ node.getIdentifier() + "']");
			return node;
		}
	}
	...
	public com.ilimi.graph.dac.model.Node updateNode(String graphId, com.ilimi.graph.dac.model.Node node,
		...
		GraphDatabaseService graphDb = Neo4jGraphFactory.getGraphDb(graphId, request);
		try (Transaction tx = graphDb.beginTx()) {
			LOGGER.debug("Transaction Started For 'updateNode' Operation. | [Node ID: '" + node.getIdentifier() + "']");
			...
			tx.success();
			LOGGER.debug("Transaction For Operation 'updateNode' Completed Successfully. | [Node ID: '"
					+ node.getIdentifier() + "']");
			return node;
		}
	}
	...
	public void importNodes(String graphId, List<com.ilimi.graph.dac.model.Node> nodes, Request request) {
		GraphDatabaseService graphDb = Neo4jGraphFactory.getGraphDb(graphId, request);
		try (Transaction tx = graphDb.beginTx()) {
			LOGGER.debug("Transaction Started For 'importNodes' Operation.");
			String date = DateUtils.formatCurrentDate();
			LOGGER.debug("Date: " + date);
			for (com.ilimi.graph.dac.model.Node node : nodes) {
				LOGGER.debug("Iterating For Node ID: " + node.getIdentifier());
				Node neo4jNode = null;
				try {
					neo4jNode = Neo4jGraphUtil.getNodeByUniqueId(graphDb, node.getIdentifier());
					LOGGER.debug("Fetched Neo4J Node: " + neo4jNode.getId());
			...
			tx.success();
			LOGGER.debug("Transaction For Operation 'importNodes' Completed Successfully.");
		}
	}
	...
	public void updatePropertyValue(String graphId, String nodeId, Property property, Request request) {
		GraphDatabaseService graphDb = Neo4jGraphFactory.getGraphDb(graphId, request);
		try (Transaction tx = graphDb.beginTx()) {
			LOGGER.debug("Transaction Started For 'updatePropertyValue' Operation. | [Node ID: '" + nodeId + "']");
			String date = DateUtils.formatCurrentDate();
			Node node = getNodeByUniqueId(graphDb, nodeId);
			if (null == property.getPropertyValue()) {
				node.removeProperty(property.getPropertyName());
				LOGGER.debug("Property '" + property.getPropertyName() + "' Removed For Node ID: " + nodeId);
			} else {
				node.setProperty(property.getPropertyName(), property.getPropertyValue());
				LOGGER.debug("Property '" + property.getPropertyName() + "' Added For Node ID: " + nodeId);
			}
			...
			tx.success();
			LOGGER.debug("Transaction For Operation 'updatePropertyValue' Completed Successfully. | [Node ID: '" + nodeId
					+ "']");
		}
	}
	...
	public void updatePropertyValues(String graphId, String nodeId, Map<String, Object> metadata, Request request) {
		GraphDatabaseService graphDb = Neo4jGraphFactory.getGraphDb(graphId, request);
		try (Transaction tx = graphDb.beginTx()) {
			LOGGER.debug("Transaction Started For 'updatePropertyValues' Operation. | [Node ID: '" + nodeId + "']");
			String date = DateUtils.formatCurrentDate();
			if (null != metadata && metadata.size() > 0) {
				...
				for (Entry<String, Object> entry : metadata.entrySet()) {
					if (null == entry.getValue()) {
						...
						LOGGER.debug("Property '" + entry.getKey() + "' Removed For Node ID: " + nodeId);
					} else {
						node.setProperty(entry.getKey(), entry.getValue());
						LOGGER.debug("Property '" + entry.getKey() + "' Added For Node ID: " + nodeId);
					}
				}
				node.setProperty(AuditProperties.lastUpdatedOn.name(), date);
				if (!StringUtils.isBlank(date))
			...
			tx.success();
			LOGGER.debug("Transaction For Operation 'updatePropertyValues' Completed Successfully. | [Node ID: '"
					+ nodeId + "']");
		}
	}
	...
	public void removePropertyValue(String graphId, String nodeId, String key, Request request) {
		GraphDatabaseService graphDb = Neo4jGraphFactory.getGraphDb(graphId, request);
		try (Transaction tx = graphDb.beginTx()) {
			LOGGER.debug("Transaction Started For 'removePropertyValue' Operation. | [Node ID: '" + nodeId + "']");
			String date = DateUtils.formatCurrentDate();
			Node node = getNodeByUniqueId(graphDb, nodeId);
			node.removeProperty(key);
			LOGGER.debug("Property '" + key + "' Removed For Node ID: " + nodeId);
			node.setProperty(AuditProperties.lastUpdatedOn.name(), date);
			if (!StringUtils.isBlank(date))
				node.setProperty(GraphDACParams.versionKey.name(), Long.toString(DateUtils.parse(date).getTime()));
			tx.success();
			LOGGER.debug("Transaction For Operation 'removePropertyValue' Completed Successfully. | [Node ID: '" + nodeId
					+ "']");
		}
	}
	...
	public void removePropertyValues(String graphId, String nodeId, List<String> keys, Request request) {
		GraphDatabaseService graphDb = Neo4jGraphFactory.getGraphDb(graphId, request);
		try (Transaction tx = graphDb.beginTx()) {
			LOGGER.debug("Transaction Started For 'removePropertyValues' Operation. | [Node ID: '" + nodeId + "']");
			String date = DateUtils.formatCurrentDate();
			Node node = getNodeByUniqueId(graphDb, nodeId);
			for (String key : keys) {
				node.removeProperty(key);
			...
			if (!StringUtils.isBlank(date))
				node.setProperty(GraphDACParams.versionKey.name(), Long.toString(DateUtils.parse(date).getTime()));
			tx.success();
			LOGGER.debug("Transaction For Operation 'removePropertyValues' Completed Successfully. | [Node ID: '"
					+ nodeId + "']");
		}
	}
	...
	public void deleteNode(String graphId, String nodeId, Request request) {
		GraphDatabaseService graphDb = Neo4jGraphFactory.getGraphDb(graphId, request);
		try (Transaction tx = graphDb.beginTx()) {
			LOGGER.debug("Transaction Started For 'deleteNode' Operation. | [Node ID: '" + nodeId + "']");
			Node node = getNodeByUniqueId(graphDb, nodeId);
			...
			node.delete();
			tx.success();
			LOGGER.debug("Transaction For Operation 'deleteNode' Completed Successfully. | [Node ID: '" + nodeId + "']");
		}
	}
	...
	public com.ilimi.graph.dac.model.Node upsertRootNode(String graphId, Request request) {
		...
		try (Transaction tx = graphDb.beginTx()) {
			LOGGER.debug("Transaction Started For 'upsertRootNode' Operation");
			...
			LOGGER.debug("Transaction For Operation 'upsertRootNode' Completed Successfully");
		}
		return node;
	}
