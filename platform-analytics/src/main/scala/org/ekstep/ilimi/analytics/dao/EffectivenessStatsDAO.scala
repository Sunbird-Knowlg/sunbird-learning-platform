...
object EffectivenessStatsDAO extends BaseDAO {
    ...
    def saveConceptEffectivness(userScores: Array[(String, (Float, Float, Float, Float))], gameId: String, conceptId: String) {
        ...
        insertSQL = "INSERT INTO CONCEPT_STATS (MIN_IMPROVEMENT, MAX_IMPROVEMENT, MEAN_IMPROVEMENT, SD, EFFECT_SIZE, CONCEPT_ID, GAME_ID) VALUES (?, ?, ?, ?, ?, ?, ?)";
        updateSQL = "UPDATE CONCEPT_STATS SET MIN_IMPROVEMENT = ?, MAX_IMPROVEMENT = ?, MEAN_IMPROVEMENT = ?, SD = ?, EFFECT_SIZE = ? WHERE CONCEPT_ID = ? and GAME_ID = ?";
    ...
    def saveRateOfAdvStats(userScores: Array[(String, Map[String, (Long, Long)])]) {
        ...
        insertSQL = "INSERT INTO GAME_STATS (MEAN_TIME_TO_MASTER, LEVELS, SD_TIME_TO_MASTER, GAME_ID) VALUES (?, ?, ?, ?)";
        updateSQL = "UPDATE GAME_STATS SET MEAN_TIME_TO_MASTER = ?, LEVELS = ?, SD_TIME_TO_MASTER = ? WHERE GAME_ID = ?";
        data = new ListBuffer[Array[AnyRef]]();
        var gameMap: Map[String, Buffer[Long]] = Map();
        var stats:DescriptiveStatistics = null;
        userScores.foreach(x =>
            x._2.foreach(f => {
                if (!gameMap.contains(f._1)) {
                    gameMap(f._1) = new ListBuffer[Long]();
                }
                gameMap(f._1) += f._2._2;
            }))
        gameMap.foreach(f => {
            stats = new DescriptiveStatistics();
            f._2.foreach { x => stats.addValue(x); }
            val mean = stats.getMean();
            val sd = stats.getStandardDeviation();
            val tstat = mean / sd;
