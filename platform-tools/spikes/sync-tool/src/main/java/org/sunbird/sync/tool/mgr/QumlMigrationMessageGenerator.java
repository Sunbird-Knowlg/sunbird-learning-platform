...
@Component
public class QumlMigrationMessageGenerator {

    private ControllerUtil util = new ControllerUtil();
    private static int batchSize = 50;
    private ObjectMapper mapper = new ObjectMapper();
    private static String actorId = "quml-migration";
    ...
    private static String action = "quml-migration";
    private static String migrationTopicName = Platform.config.getString("quml.migration.request.topic");

    @PostConstruct
    private void init() throws Exception {
        int batch = Platform.config.hasPath("quml.migration.batch.size") ? Platform.config.getInt("quml.migration.batch.size") : 50;
        batchSize = batch;
    }

    public void generateMgrMsg(String graphId, String[] objectTypes, String[] status, String[] contentIds, double migrationVersion, Integer limit, Integer delay) throws Exception {
        ...
        System.out.println("-----------------------------------------");
        System.out.println("\nQuML Migration Event Generation starting at " + startTime);
        Map<String, Long> counts = util.getQumlMigrationObjectCount(graphId, Arrays.asList(objectTypes), statusList, contentIdsList, migrationVersion);
        if (counts.isEmpty()) {
            ...
            for (String objectType : objTypes) {
                ...
                System.out.println(count + " - " + objectType + " nodes available for quml migration");
            }
        }
        for (String objectType : objectTypes) {
            ...
            if (count > 0) {
                ...
                System.out.println("QumlMigrationMessageGenerator:: generateMgrMsg:: stopLimit: " + stopLimit + " || total: " + total);

                boolean found = true;
                while (found && start < stopLimit) {
                    List<Node> nodes = null;
                    try {
                        nodes = util.getNodes(graphId, objectType.trim(), statusList, contentIdsList, migrationVersion, start, batchSize);
                    ...
                    if (CollectionUtils.isNotEmpty(nodes)) {
                        System.out.println("QumlMigrationMessageGenerator:: generateMgrMsg:: nodes: " + nodes.size());
                        start += batchSize;
                        Map<String, String> events = generateMigrationEvent(nodes, errors);
                        System.out.println("QumlMigrationMessageGenerator:: generateMgrMsg:: events: " + events.size());
                        ...
                        System.out.println("QumlMigrationMessageGenerator:: generateMgrMsg:: Breaking Event Generation Loop!");
                        found = false;
                        break;
                    }
                }
                ...
                System.out.println("\nQuML Migration Event Generation completed for object of type " + objectType + " in: " + (endTime - startTime) + "ms");
            } else {
        ...
        System.out.println("-----------------------------------------");
        long endTime = System.currentTimeMillis();
        System.out.println("QuML Migration Event Generation completed at " + endTime);
    ...
    private String getEvent(Node node, Map<String, String> errors) {
        ...
        Map<String, Object> edata = new HashMap<String, Object>() {{
            ...
            put("metadata", new HashMap<String, Object>() {{
                ...
                put("qumlVersion", node.getMetadata().get("qumlVersion"));
                put("schemaVersion", node.getMetadata().get("schemaVersion"));
