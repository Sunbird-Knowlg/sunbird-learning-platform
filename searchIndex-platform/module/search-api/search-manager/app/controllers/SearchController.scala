...
class SearchController@Inject()(cc: ControllerComponents, actorSystem: ActorSystem)(implicit exec: ExecutionContext) extends BaseController(cc) {
    ...
    def search() = Action.async { implicit request =>
        ...
        getResult(mgr.search(internalReq))
    }
    ...
    def count() = Action.async { implicit request =>
        ...
        val internalReq = getRequest(apiId)
        getResult(mgr.count(internalReq))
    }
}
