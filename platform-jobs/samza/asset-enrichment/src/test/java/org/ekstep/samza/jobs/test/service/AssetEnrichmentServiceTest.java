...
public class ImageTaggingServiceTest {
    ...
    public void setUp() throws Exception {
        ...
        imageTaggingService = Mockito.spy(new ImageTaggingService(controllerUtil,jobRequest));
        doNothing().when(imageTaggingService).initialize(configMock);
        imageTaggingTask = new ImageTaggingTask(configMock, contextMock);
    ...
    public void pushStreamingUrlSuccessTest() {
        ...
        try{
            imageTaggingTask.process(envelopeMock, collectorMock, coordinatorMock);
        } catch (Exception e){
            e.printStackTrace();
        }
    ...
    public void pushStreamUrlSuccess() {
        ...
        String videoUrl = (String) expectedNode.getMetadata().get(ImageWorkflowEnums.artifactUrl.name());
        imageTaggingService.pushStreamingUrlRequest(expectedNode, videoUrl);
    }
