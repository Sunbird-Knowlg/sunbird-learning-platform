...
public class MVCProcessorIndexer extends AbstractESIndexer {
	...
	String arr[];
    String contentreadapi = "", mlworkbenchapirequest = "", mlvectorListRequest = "" , jobname = "" , mlkeywordapi = "" , mlvectorapi = "" , source_url = null , label;
	...
	Map<String,Object> insertintoCassandra(Map<String,Object> obj,String identifier) {
		...
		if(StringUtils.isNotBlank(action)) {
			if(action.equalsIgnoreCase("update-es-index")) {
				...
				cqlquery = "UPDATE content_data SET textbook_name = ? ,level1_concept = ? , level2_concept = ? , level3_concept = ? , label = ? , level1_name = ? , level2_name = ? , level3_name = ? , source = ? , source_url = ? WHERE content_id = ?";
				PreparedStatement prepared = preparestaement(session,cqlquery);
				 bound = prepared.bind(textbook_name,ml_level1,ml_level2,ml_level3,label,level1_name ,level2_name,level3_name,source,source_url,identifier);
