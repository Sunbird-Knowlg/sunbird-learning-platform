...
public class RepublishPipelineService implements ISamzaService {

	static JobLogger LOGGER = new JobLogger(RepublishPipelineService.class);
	...
	public void processMessage(Map<String, Object> message, JobMetrics metrics, MessageCollector collector)
		...
		Map<String, Object> edata = (Map<String, Object>) message.get(RepublishPipelineParams.edata.name());
		Map<String, Object> object = (Map<String, Object>) message.get(RepublishPipelineParams.object.name());
		...
		String nodeId = (String) object.get(RepublishPipelineParams.id.name());
	...
	private void processJob(Map<String, Object> edata, String contentId, JobMetrics metrics) throws Exception {
		...
		String publishType = (String) edata.get(RepublishPipelineParams.publish_type.name());
		node.getMetadata().put(RepublishPipelineParams.publish_type.name(), publishType);
		if (publishContent(node, publishType)) {
			metrics.incSuccessCounter();
			edata.put(RepublishPipelineParams.status.name(), RepublishPipelineParams.SUCCESS.name());
			LOGGER.debug("Node publish operation :: SUCCESS :: For NodeId :: " + node.getIdentifier());
		} else {
			edata.put(RepublishPipelineParams.status.name(), RepublishPipelineParams.FAILED.name());
	...
	private void prePublishUpdate(Map<String, Object> edata, Node node) {
		...
		edata.put(RepublishPipelineParams.status.name(), RepublishPipelineParams.Processing.name());
	...
	private Node getNode(String nodeId) {
		...
		node = util.getNode(RepublishPipelineParams.domain.name(), imgNodeId);
		if (null == node) {
			node = util.getNode(RepublishPipelineParams.domain.name(), nodeId);
		}
		return node;
	}
	...
	private boolean publishContent(Node node, String publishType) throws Exception {
		...
		if (StringUtils.equalsIgnoreCase((String) node.getMetadata().get(RepublishPipelineParams.mimeType.name()),
		...
		latestNode.getMetadata().put(RepublishPipelineParams.publish_type.name(), publishType);
		publishNode(latestNode, (String) latestNode.getMetadata().get(RepublishPipelineParams.mimeType.name()));
		
		Node publishedNode = getNode(latestNode.getIdentifier().replace(".img", ""));
		if (StringUtils.equalsIgnoreCase((String) publishedNode.getMetadata().get(RepublishPipelineParams.status.name()),
				RepublishPipelineParams.Failed.name()))
			return false;

		if (StringUtils.equalsIgnoreCase(
				((String) publishedNode.getMetadata().get(RepublishPipelineParams.mimeType.name())),
				COLLECTION_CONTENT_MIMETYPE)) {
			String versionKey = Platform.config.getString(DACConfigurationConstants.PASSPORT_KEY_BASE_PROPERTY);
			publishedNode.getMetadata().put(RepublishPipelineParams.versionKey.name(), versionKey);
	...
	private void publishNode(Node node, String mimeType) {
		...
		try {
			...
			parameterMap.put(RepublishPipelineParams.node.name(), node);
			parameterMap.put(RepublishPipelineParams.ecmlType.name(), PublishManager.isECMLContent(mimeType));
			...
			pipeline.init(RepublishPipelineParams.publish.name(), parameterMap);
		} catch (Exception e) {
			e.printStackTrace();
			LOGGER.info(
	...
	private void setContentBody(Node node, String mimeType) {
		if (PublishManager.isECMLContent(mimeType)) {
			node.getMetadata().put(RepublishPipelineParams.body.name(),
	...
	private boolean validateObject(Map<String, Object> edata) {
		...
		if (!StringUtils.equalsIgnoreCase((String) edata.get(RepublishPipelineParams.contentType.name()),
				RepublishPipelineParams.Asset.name())) {
			if (((Integer) edata.get(RepublishPipelineParams.iteration.name()) <= getMaxIterations()))
				return true;
		}
		return false;
	}
	...
	private void processCollection(Node node) throws Exception {
		...
		if (StringUtils.equalsIgnoreCase((String) node.getMetadata().get(RepublishPipelineParams.visibility.name()),
				RepublishPipelineParams.Default.name())) {
	...
	private List<String> getChildren(Node node) throws Exception {
		...
		if (null != node.getOutRelations()) {
			for (Relation rel : node.getOutRelations()) {
				if (RepublishPipelineParams.content.name().equalsIgnoreCase(rel.getEndNodeObjectType())) {
	...
	private Map<String, Object> processChild(Node node) throws Exception {
		...
		if (null != node.getMetadata().get(RepublishPipelineParams.domain.name())) {
			String[] domainData = (String[]) node.getMetadata().get(RepublishPipelineParams.domain.name());
			domain = new HashSet<Object>(Arrays.asList(domainData));
			result.put("domain", domain);
		}
		if (null != node.getMetadata().get(RepublishPipelineParams.gradeLevel.name())) {
			String[] gradeData = (String[]) node.getMetadata().get(RepublishPipelineParams.gradeLevel.name());
			grade = new HashSet<Object>(Arrays.asList(gradeData));
			result.put("gradeLevel", grade);
		}
		if (null != node.getMetadata().get(RepublishPipelineParams.ageGroup.name())) {
			String[] ageData = (String[]) node.getMetadata().get(RepublishPipelineParams.ageGroup.name());
			age = new HashSet<Object>(Arrays.asList(ageData));
			result.put(RepublishPipelineParams.ageGroup.name(), age);
		}
		if (null != node.getMetadata().get(RepublishPipelineParams.medium.name())) {
			String mediumData = (String) node.getMetadata().get(RepublishPipelineParams.medium.name());
			medium = new HashSet<Object>(Arrays.asList(mediumData));
			result.put(RepublishPipelineParams.medium.name(), medium);
		}
		if (null != node.getMetadata().get(RepublishPipelineParams.subject.name())) {
			String subjectData = (String) node.getMetadata().get(RepublishPipelineParams.subject.name());
			subject = new HashSet<Object>(Arrays.asList(subjectData));
			result.put(RepublishPipelineParams.subject.name(), subject);
		}
		if (null != node.getMetadata().get(RepublishPipelineParams.genre.name())) {
			String[] genreData = (String[]) node.getMetadata().get(RepublishPipelineParams.genre.name());
			genre = new HashSet<Object>(Arrays.asList(genreData));
			result.put(RepublishPipelineParams.genre.name(), genre);
		}
		if (null != node.getMetadata().get(RepublishPipelineParams.theme.name())) {
			String[] themeData = (String[]) node.getMetadata().get(RepublishPipelineParams.theme.name());
			theme = new HashSet<Object>(Arrays.asList(themeData));
			result.put(RepublishPipelineParams.theme.name(), theme);
		}
		if (null != node.getMetadata().get(RepublishPipelineParams.keywords.name())) {
			String[] keyData = (String[]) node.getMetadata().get(RepublishPipelineParams.keywords.name());
			keywords = new HashSet<Object>(Arrays.asList(keyData));
			result.put(RepublishPipelineParams.keywords.name(), keywords);
		}
		for (Relation rel : node.getOutRelations()) {
		...
		if (null != concepts && !concepts.isEmpty()) {
			result.put(RepublishPipelineParams.concepts.name(), concepts);
		}
		return result;
	}
	...
	private Integer getLeafNodeCount(Map<String, Object> data, int leafCount) {
		...
		if (null != children && !children.isEmpty()) {
			...
			if (!COLLECTION_CONTENT_MIMETYPE.equals(data.get(RepublishPipelineParams.mimeType.name())))
				leafCount++;
		}
		return leafCount;
	}
