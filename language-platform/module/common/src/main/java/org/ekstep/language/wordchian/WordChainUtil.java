...
public class WordChainUtil {
	...
	private IWordSet getWordSet(String languageId, Node node, WordComplexity wc){
		if(languageId.equalsIgnoreCase("en")){
			return new EnglishLanguageWordSet(languageId, node, wc);
		}else{
			return new IndianLanguageWordSet(languageId, node, wc);
		}
	}
	...
	public void updateWordSet(String languageId, Node node, WordComplexity wc) throws Exception{
		...
		if("Live".equalsIgnoreCase(status)){
			IWordSet wordSet = getWordSet(languageId, node, wc);
			wordSet.addRhymingSound();
			wordSet.addAkshara();
			Node updatedNode = wordUtil.getDataNode(languageId, node.getIdentifier());
			node.setInRelations(updatedNode.getInRelations());
			wordUtil.updateWord(updatedNode, languageId, updatedNode.getIdentifier());
		}else{
