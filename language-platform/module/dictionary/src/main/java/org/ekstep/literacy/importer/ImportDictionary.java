...
//	}
	...
	protected void importIndoWordNetData(String languageId, SourceType sourceType, String csvFileUrl) {
		...
		try {
			...
			while ((line = br.readLine()) != null) {
				...
				// synset.setIdentifier(languageId + ':' + "S:" + synsetDetails[899]);
				...
				synset.setHypernymSynsetId(getSynsetIdByRelation(HYPERNYM_CODE, line, ObjectType.Synset, languageId));
				...
				for (int i = 0; i < synsetMemberWordList.length; i++) {
					...
					if (results.size() <= 0) {
						...
						CSVMemeberWordsId += (languageId + ':' + "W:" + synsetDetails[IDX_ID_IN_SOURCE] + i) + CSV_SEPARATOR;
						...
						word.setAntonymOfWordId(getSynsetIdByRelation(ANTONYM_CODE, line, ObjectType.Word, languageId));
						word.setHyponymOfWordId(getSynsetIdByRelation(HYPONYM_CODE, line, ObjectType.Word, languageId));
    ...
    private static void writeSynsetsToCSV(List<SynsetModel> synsetList)
    {
        try
        {
            ...
            BufferedWriter bw = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(fileName), "UTF-8"));
            bw.write("identifier, rel:hasMember, rel:hasAntonymOf, rel:hasHyponym, rel:hasMeronym, rel:hasHolonym, rel:hasHypernym, gloss, exampleUsage, POS");
            bw.newLine();
            for (SynsetModel synset : synsetList)
            {
                ...
                oneLine.append(CSV_SEPARATOR);
                oneLine.append(synset.getHyponymSynsetId() == null ? "" : synset.getHyponymSynsetId());
                oneLine.append(CSV_SEPARATOR);
                oneLine.append(synset.getMeronymSynsetId() == null ? "" : synset.getMeronymSynsetId());
                oneLine.append(CSV_SEPARATOR);
                oneLine.append(synset.getHolonymSynsetId() == null ? "" : synset.getHolonymSynsetId());
                oneLine.append(CSV_SEPARATOR);
                ...
                oneLine.append(CSV_SEPARATOR);
                oneLine.append(synset.getUsage() == null ? "" : synset.getUsage());
                oneLine.append(CSV_SEPARATOR);
                oneLine.append(synset.getPartOfSpeech() == null ? "" : synset.getPartOfSpeech());
                bw.write(oneLine.toString());
                bw.newLine();
            }
            bw.flush();
	...
	protected String getSynsetIdByRelation(String relationId, String synsetDetail, ObjectType objectType, String languageId ) {
		try {
			String id = null;
